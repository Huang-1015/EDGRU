...
class TrainSetLoader(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            raise FileNotFoundError(f"Could not find image or mask for {self.train_list[idx]}")
...
class TestSetLoader(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            print(f"Error loading image or mask: {e}")
            raise FileNotFoundError(f"Could not find image or mask for {self.test_list[idx]}")
...
class EvalSetLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask_gt = Image.open(self.dataset_dir + '/masks/' + self.test_list[idx] + '_pixels0.png')
