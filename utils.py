...
def get_img_norm_cfg(dataset_name, dataset_dir):
    if  dataset_name == 'NUAA-SIRST':
    ...
    elif dataset_name == 'groundbird':
        img_norm_cfg = {'mean': 102.69807434082031, 'std': 35.63002014160156}
    elif dataset_name == 'groundbird1':
        img_norm_cfg = {'mean': 108.79157257080078, 'std': 40.473453521728516}
    else:
        ...
        print('mean: ' + str(float(np.array(mean_list).mean())) + '\nstd: ' + str(float(np.array(std_list).mean())))
    return img_norm_cfg
...
def get_optimizer(net, optimizer_name, scheduler_name, optimizer_settings, scheduler_settings):
    if optimizer_name == 'Adam':
        ...
        optimizer  = torch.optim.Adagrad(net.parameters(), lr=optimizer_settings['lr'], weight_decay=1e-3)
    elif optimizer_name == 'SGD':
        optimizer  = torch.optim.SGD(net.parameters(), lr=optimizer_settings['lr'], momentum=0.9, weight_decay=1e-3)
    ...
    if scheduler_name == 'MultiStepLR':
        ...
        scheduler  = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=1000, eta_min=scheduler_settings['min_lr'])
    elif scheduler_name == 'ReduceLROnPlateau':
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=2, factor=0.95, mode='min')
